...
import { CalendarDays, MessageSquare, Pencil, Phone, Plus, Search, Trash2, Wrench } from "lucide-react";
...
export function CustomersScreen({ selectedId }: { selectedId?: number }) {
  ...
  useEffect(() => {
    Promise.all([api.customers.index(), api.bookings.index()])
      .then(([customerRecords, bookingRecords]) => {
        ...
        if (!selectedId && customerRecords[0]) router.replace(`/customers/${customerRecords[0].id}`);
      })
  ...
  const customerBookings = selected ? bookings.filter((booking) => booking.customerId === selected.id) : [];
  const customerConversation = selected ? conversations.find((conversation) => conversation.customerId === `c${selected.id}`) : undefined;
  ...
  if (loading) return <div className="flex h-full items-center justify-center text-sm text-zinc-500">Loading customers…</div>;
        ...
        <div className="grid min-h-0 flex-1 md:grid-cols-[320px_minmax(0,1fr)]">
          <div className="flex min-h-0 flex-col border-r border-zinc-200 bg-white">
            <div className="flex gap-2 p-4 pb-2">
              ...
              <Button size="icon" aria-label="Add customer" onClick={() => { setEditingCustomer(undefined); setCustomerDialogOpen(true); }}><Plus /></Button>
            </div>
            ...
            {selected ? (
              <div className="mx-auto max-w-4xl p-5 sm:p-8">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-xl font-semibold tracking-[-0.02em]">{selected.name}</h2><p className="mt-1 text-sm text-muted-foreground">{customerSince(selected)}</p></div>
                  <Button variant="outline" size="icon" aria-label="Edit customer" onClick={() => { setEditingCustomer(selected); setCustomerDialogOpen(true); }}><Pencil /></Button>
                  <Button variant="outline" size="icon" aria-label="Delete customer" onClick={() => void removeCustomer()}><Trash2 /></Button>
                  <Button onClick={() => { setBookingCustomer(selected); setBookingDialogOpen(true); }}><Plus /> Book service</Button>
                </div>
                <div className="mt-8 grid gap-5 lg:grid-cols-2">
                  ...
                  <section className="rounded-xl border border-zinc-200 bg-white p-5">
                    <div className="flex items-center justify-between"><h3 className="text-sm font-semibold">Notes</h3><Button variant="ghost" size="icon-sm" aria-label="Edit customer notes" onClick={() => { setEditingCustomer(selected); setCustomerDialogOpen(true); }}><Pencil /></Button></div>
                    <p className="mt-4 text-sm leading-6 text-zinc-600">{selected.notes || "No notes yet."}</p>
                  </section>
                </div>
                  ...
                  {customerBookings.length ? customerBookings.map((booking, index) => {
                    ...
                    return (
                      <div key={booking.id} className={cn("flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center", index !== 0 && "border-t border-zinc-200")}>
                        ...
                        <div className="min-w-0 flex-1"><div className="text-sm font-medium">{booking.service}</div><div className="mt-1 text-xs text-muted-foreground">{date.day}, {date.date} · {date.time} · {booking.staff}</div></div>
                ...
                <section className="mt-5 rounded-xl border border-zinc-200 bg-white p-5">
                  ...
                  {customerConversation ? <div className="mt-4 flex gap-3 rounded-lg bg-zinc-50 p-4"><MessageSquare className="mt-0.5 size-4 shrink-0 text-zinc-500" /><div><div className="text-sm leading-6">{customerConversation.preview}</div><div className="mt-1 text-xs text-muted-foreground">{customerConversation.channel} · {customerConversation.time} ago</div></div></div> : <p className="mt-4 text-sm text-muted-foreground">No recent conversation.</p>}
                </section>
              </div>
            ) : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Add your first customer to get started.</div>}
          </ScrollArea>
        </div>
      </div>
      {customerDialogOpen ? <CustomerDialog onOpenChange={setCustomerDialogOpen} customer={editingCustomer} onSave={saveCustomer} /> : null}
      {bookingDialogOpen ? <NewBookingDialog onOpenChange={(open) => { setBookingDialogOpen(open); if (!open) setBookingCustomer(undefined); }} customers={customers} customer={bookingCustomer} onSave={saveBooking} /> : null}
    </>
  );
}
